            ...
            {quotaError && (
              quotaError.retryAfter
                ? <RetryCountdown
                    ...
                    onRetry={() => { setQuotaError(null); startReading(); }}
                    onDismiss={reset}
                  />
                : <div className="w-full rounded-2xl border border-amber-500/20 bg-amber-900/10 px-6 py-5 flex flex-col gap-3">
                    ...
                    <button onClick={reset} className="text-slate-500 text-xs hover:text-slate-300 cursor-pointer text-left">返回重新测算 →</button>
                  </div>
            )}
